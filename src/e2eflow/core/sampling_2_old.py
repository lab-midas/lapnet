...
import e2eflow.VDPD as VDPD


def generate_mask_2(subsampleType=2, acc=4, vd_type=4, nRep=4):
    lCompile = False
    subsampleType = subsampleType  # Poisson-Disc {1}, Gaussian {2}
    numLin = 256  # ky points
    numPar = 72  # kz points
    acc = acc  # acceleration
    nCenter = 6  # percentage of fully sampled center region
    vd_type = vd_type  # variable-density options: none {1}, central point {2}, central block {3}, central ellipse {4}, local adaptive variable-density (needs external fraction.txt file) {5}
    ...
    nRep = nRep  # number of repetitions
    ...
    if lCompile:
        ...
        shutil.copyfile("build/lib.linux-x86_64-3.5/VDPD.cpython-35m-x86_64-linux-gnu.so", "./VDPD.cpython-35m-x86_64-linux-gnu.so")


    parameter_list = np.asarray([numLin, numPar, acc, subsampleType, nCenter/100, pF_value, pF_x, nRep, vd_type, smpl_type, ellip_mask, p, n, body_region, iso_fac], dtype='float32')
    ...
    #
    #     print('#samples repetition {} = {}\n'.format(iRep, np.sum(mask_rep[:, :, iRep-1], axis=(0, 1))))
    #
    # plt.show()
    return mask_rep
