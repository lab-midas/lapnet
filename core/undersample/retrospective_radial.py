...
def get_radial_kspace(img, acc):
    Nd = np.shape(img)
    Kd = np.shape(img)
    Jd = (1, 1)

    maxsize = np.amax(np.shape(img))

    nyquist_spokes = np.round(np.pi / 2 * maxsize)

    n_spokes = int(np.round(nyquist_spokes / acc))

    spoke_range = (np.arange(0, n_spokes) - (int(n_spokes / 2))) * np.pi / (
        int(n_spokes / 2))  # normalized between -pi and pi
    M = n_spokes * 360
    om = np.zeros((M, 2), dtype=np.float32)

    for angle in np.arange(0, 360):
        radian = angle * 2 * np.pi / 360.0
        spoke_x = spoke_range * np.cos(radian)
        spoke_y = spoke_range * np.sin(radian)
        om[n_spokes * angle: n_spokes * (angle + 1), 0] = spoke_x
        om[n_spokes * angle: n_spokes * (angle + 1), 1] = spoke_y
...
def test_k_radial(sinpath, acc):
    indata = sio.loadmat(sinpath)

    img_cart = np.ascontiguousarray(indata['dFixed'].astype(np.float32))

    fully_k = get_radial_kspace(img_cart[:, :, 30], 1)

    plt.imshow(np.log(np.abs(fully_k)), cmap='gray')
    plt.axis('off')
    plt.set_title(f'fully sampled')
    plt.show()

    img_rad = subsample_radial(img_cart[..., None], acc, None)
    sub_k = get_radial_kspace(img_rad[..., 30, 0], acc)
    plt.imshow(np.log(np.abs(sub_k)), cmap='gray')
    plt.axis('off')
    plt.set_title(f'x{acc}')
    plt.show()


if __name__ == "__main__":
    sinpath = 'card_radial.mat'
    indata = sio.loadmat(sinpath)
