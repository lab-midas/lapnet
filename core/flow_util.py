...
def flow_to_color(flow, mask=None, max_flow=None):
    ...
    """
    n = 8
    num_batch, height, width, _ = np.unstack(np.shape(flow))
    mask = np.ones([num_batch, height, width, 1]) if mask is None else mask
    flow_u, flow_v = np.unstack(flow, axis=3)
    if max_flow is not None:
        max_flow = np.maximum(max_flow, 1)
    else:
        max_flow = np.reduce_max(np.abs(flow * mask))
    mag = np.sqrt(np.reduce_sum(np.square(flow), 3))
    angle = atan2(flow_v, flow_u)

    im_h = np.mod(angle / (2 * np.pi) + 1.0, 1.0)
    im_s = np.clip_by_value(mag * n / max_flow, 0, 1)
    im_v = np.clip_by_value(n - im_s, 0, 1)
    im_hsv = np.stack([im_h, im_s, im_v], 3)
    im = np.image.hsv_to_rgb(im_hsv)
    return im * mask
...
def flow_error_image(flow_1, flow_2, mask_occ, mask_noc=None, log_colors=True):
    ...
    """
    mask_noc = np.ones(np.shape(mask_occ)) if mask_noc is None else mask_noc
    diff_sq = (flow_1 - flow_2) ** 2
    diff = np.sqrt(np.reduce_sum(diff_sq, [3], keepdims=True))
    if log_colors:
        num_batch, height, width, _ = np.unstack(np.shape(flow_1))
        colormap = [
            [0, 0.0625, 49, 54, 149],
        ...
        mag = np.sqrt(np.reduce_sum(np.square(flow_2), 3, keepdims=True))
        error = np.minimum(diff / 3, 20 * diff / mag)
        im = np.zeros([num_batch, height, width, 3])
        for i in range(colormap.shape[0]):
            colors = colormap[i, :]
            cond = np.logical_and(np.greater_equal(error, colors[0]),
                                  np.less(error, colors[1]))
            im = np.where(np.tile(cond, [1, 1, 1, 3]),
                          np.ones([num_batch, height, width, 1]) * colors[2:5],
                          im)
        im = np.where(np.tile(np.cast(mask_noc, np.bool), [1, 1, 1, 3]),
                      im, im * 0.5)
        im = im * mask_occ
    else:
...
def outlier_ratio(gt_flow, flow, mask, threshold=3.0, relative=0.05):
    ...
    if relative is not None:
        threshold = np.maximum(threshold, euclidean(gt_flow) * relative)
        outliers = np.cast(np.greater_equal(diff, threshold), np.float32)
    else:
        outliers = np.cast(np.greater_equal(diff, threshold), np.float32)
    ratio = np.reduce_sum(outliers) / np.reduce_sum(mask)
    return ratio
...
def euclidean(t):
    return np.sqrt(np.reduce_sum(t ** 2, [3], keepdims=True))
